...
function JobListItem({ job }) {
  ...
  console.log(job.sponsored)
  return (
    < div className="mt-3 border border-gray-200 px-3 py-3 shadow-sm rounded-sm" >
        ...
        <div className="flex items-center">
          ...
          <NavLink to={`/jobs/${job.id}`}>
            <h3 className="text-2xl text-gray-800 font-semibold">
              {job.title}
              {job.sponsored && (
                <span className="bg-green-100 ml-2 text-sm text-green-600 px-2 py-2 rounded-md">Sponsord</span>
  ...
  useEffect(() => {
    function fetchJobs() {
      axios.get(API.jobs.list, {
        ...
        .then(res => {
          const sponsordjobs = res.data.filter(job => job.sponsored)
          const restofjobs = res.data.filter(job => !job.sponsored)

          setJobs(restofjobs)
          setsponsoredJobs(sponsordjobs)
        })
    }
    fetchJobs()
    return () => null
